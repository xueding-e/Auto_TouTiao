...
def extract_article_images(url, min_size=400):
    ...
    try:
        ...
        # 跳过明显的小图和图标
        skip_keywords = [
            'icon', 'logo', 'avatar', 'emoji', '1x1', 'pixel',
            'qr_code', 'favicon', 'w85h85', 'w50h50', 'w32h32', 'w16h16',
        ]
...
def extract_images_from_html(html_text, max_images=3):
    ...
    for match in img_pattern.finditer(html_text):
        url = match.group(1)
        # 跳过明显的小图、图标、logo
        if any(x in url.lower() for x in ['icon', 'logo', 'avatar', 'emoji', '1x1', 'pixel', 'qr_code']):
            continue
...
def search_baidu_images(keyword, count=3):
    ...
    try:
        ...
        url = (
            ...
            f"&adpicid=&st=-1&z=&ic=0&hd=1&latest=1&copyright=0"
            f"&word={encoded_keyword}&s=&se=&tab=&width=1024&height=768"
...
def search_bing_images(keyword, count=3):
    ...
    try:
        encoded_keyword = urllib.parse.quote(keyword)
        # filterui:imagesize-large 限定只搜索大尺寸图片
        fetch_count = count * 3
        url = (
            f"https://www.bing.com/images/async?q={encoded_keyword}"
...
def extract_topic_keywords(topic):
    ...
    stop_words = {
        ...
        '他', '这', '中', '大', '来', '上', '国', '个', '到', '说', '们', '为',
        '子', '你', '地', '一个', '没有', '什么', '自己', '被', '把', '但',
        '新', '也', '与', '及', '等', '或', '从', '已', '将', '对', '以',
    }
    keywords = []
